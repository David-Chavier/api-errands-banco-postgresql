...
describe("testando a listagem dos recados", () => {
  ...
  test("Deveria retornar erro 401 se errandid não for um uuid", async () => {
    const result = await supertest(sut)
      .get(`/user/9385c62c-b047-413a-9b06-390cd7186a1d/errand/dasfsdf`)
      .send();
  ...
  test("Deveria retornar erro 401 se o usuario não for encontrado no banco de dados", async () => {
    const result = await supertest(sut)
      .get(
        `/user/9385c62c-b047-413a-9b06-390cd7186a1d/errand/9385c62c-b047-413a-9b06-390cd7186a1d`
      )
      .send();
  ...
  test("Deveria retornar 200 se os recados forem listados com sucesso do banco cache", async () => {
    ...
    await createErrandBancoRelacional(errand);

    const listErrand = await new ErrandsRepository().list({
      userid: listUser[0].userId,
    });

    await createErrand(listErrand[0].id, listErrand[0]);

    const result = await supertest(sut)
      .get(`/user/${listUser[0].userId}/errand/${listErrand[0].id}`)
      .send();
    ...
    expect(result).toHaveProperty("body.ok");
    expect(result.body.message).toBe("Errand requested cache");
    expect(result.body.ok).toBe(true);
  });

  test("Deveria retornar erro 400 se o recado não for encontrado no banco relacional", async () => {
    const user = new User("any_username", "123456789");
    await createUser(user);
    const listUser = await new UserRepository().list();
  ...
  test("Deveria retornar 200 se os recados forem listados com sucesso do banco relacional", async () => {
    ...
    expect(result).toHaveProperty("status", 200);
    expect(result).toHaveProperty("body.ok");
    expect(result.body.message).toBe("Errand requested");
    expect(result.body.ok).toBe(true);
  });
});
