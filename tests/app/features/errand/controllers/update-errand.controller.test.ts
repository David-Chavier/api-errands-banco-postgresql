...
describe("testando a rota de update recados", () => {
  beforeAll(async () => {
  ...
  test("Deveria retornar erro 401 se o userid do usuário não for um uuid", async () => {
    const result = await supertest(sut)
      .put(`/user/9385c62/errand/124234`)
      .send({});
  ...
  test("Deveria retornar erro 401 se errandid não for um uuid", async () => {
    const result = await supertest(sut)
      .put(`/user/9385c62c-b047-413a-9b06-390cd7186a1d/errand/dasfsdf`)
      .send();
  ...
  test("Deveria retornar erro 400 se não for passado dados a serem atualizados", async () => {
    const result = await supertest(sut)
      .put(
    ...
    expect(result).toHaveProperty("body.ok");
    expect(result.body.message).toBe("No value change was requested");
    expect(result.body.ok).toBe(false);
  });
  ...
  test("Deveria retornar 200 se o recado for atualizado com sucesso", async () => {
    ...
    const result = await supertest(sut)
      .put(`/user/${listUser[0].userId}/errand/${listErrand[0].id}`)
      .send({ description: "nova description", details: "novos details" });
    ...
    expect(result).toHaveProperty("body.ok");
    expect(result.body.message).toBe("Errand was successfully update");
